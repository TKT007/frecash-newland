  ...
  try {
    ...
    const payload = {
      ...
      properties: {
        ...
        content_id: 'apple-pay-750',
        value: event_name === 'Purchase' ? 500.00 : 1.00, // ✅ Valor correto para Purchase
        currency: 'USD'
      }
    };

    console.log('📤 Enviando para TikTok (Apple Pay):', JSON.stringify(payload, null, 2));
    ...
    console.log('✅ Resposta TikTok (Apple Pay):', response.data);

    return res.status(200).json({
      success: true,
      message: 'Conversão registrada (Apple Pay)',
      event_id: finalEventId,
      tiktok_response: response.data
    });

  } catch (error) {
    console.error('❌ Erro (Apple Pay):', error.response?.data || error.message);
