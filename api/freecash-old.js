...
module.exports = async (req, res) => {
  const TIKTOK_PIXEL_ID_FREECASH_OLD = process.env.TIKTOK_PIXEL_ID_FREECASH_OLD || 'D4DO8HRC77UCI3HO4RHG';
  const TIKTOK_ACCESS_TOKEN_FREECASH_OLD = process.env.TIKTOK_ACCESS_TOKEN_FREECASH_OLD;
  ...
  try {
    ...
    if (!TIKTOK_ACCESS_TOKEN_FREECASH_OLD) {
    ...
    const payload = {
      pixel_code: TIKTOK_PIXEL_ID_FREECASH_OLD,
    ...
    const response = await axios.post(
      ...
      {
        headers: {
          'Access-Token': TIKTOK_ACCESS_TOKEN_FREECASH_OLD,
