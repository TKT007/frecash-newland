  ...
  try {
    ...
    const payload = {
      ...
      properties: {
        ...
        content_id: 'cash-1000',
        value: event_name === 'Purchase' ? 500.00 : 1.00, // ✅ Valor correto para Purchase
        currency: 'USD'
      }
    };

    console.log('📤 Enviando para TikTok (cash):', JSON.stringify(payload, null, 2));
    ...
    console.log('✅ Resposta TikTok (cash):', response.data);

    return res.status(200).json({
      success: true,
      message: 'Conversão registrada (cash)',
      event_id: finalEventId,
      tiktok_response: response.data
    });

  } catch (error) {
    console.error('❌ Erro (cash):', error.response?.data || error.message);
